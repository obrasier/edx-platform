...
class StudentsListWidget
  # create a StudentsListWidget `$container` is a jquery object to embody.
  # `params` holds template parameters. `params` should look like the defaults below.
  constructor: (@$container, params={}) ->
    params = _.defaults params,
      title: "Students List"
      info: """
        Use this list to manage Students.
      """
...
class Auth2ListWidget extends StudentsListWidget
  constructor: ($container, @rolename, @$error_section) ->
    ...
    unless @rolename?
      throw "Auth2ListWidget missing @rolename"
  ...
  add_handler: (input) ->
    if input? and input isnt ''
      @modify_student_access input, 'allow', (error) =>
        # abort on error
  ...
  reload_list: ->
    # @clear_rows()
    @get_student_list (error, student_list) =>
      # abort on error
      return @show_errors error unless error is null

      # only show the list of there are studentss
      @clear_rows()

      # use _.each instead of 'for' so that students
      # is bound in the button callback.
      _.each student_list, (student) =>
        # if there are students, show the list
        ...
        $revoke_btn = $ _.template('<div class="revoke"><i class="icon fa fa-times-circle" aria-hidden="true"></i> <%= label %></div>', {label: label_trans}),
          class: 'revoke'
        $revoke_btn.click =>
            @modify_student_access student.email, 'revoke', (error) =>
              # abort on error
              return @show_errors error unless error is null
              @clear_errors()
              @reload_list()
        @add_row [student.username, student.email, $revoke_btn]
  ...
  # send ajax request to list students
  # `cb` is called with cb(error, student_list)
  get_student_list: (cb) ->
    $.ajax
      dataType: 'json'
      url: @list_endpoint
  ...
  modify_student_access: (unique_student_identifier, action, cb) ->
    $.ajax
      dataType: 'json'
      url: @modify_endpoint
      data:
        ...
        rolename: @rolename
        action: action
      success: (data) => @student_response data
...
class BatchEnrollment
  ...
  display_response: (data_from_server) ->
    ...
    if notunenrolled.length
      `// Translators: A list of users appears after this sentence. This situation arises when a staff student tries to unenroll a user who is not currently enrolled in this course.`
...
# Wrapper for auth2 list subsection.
...
class Auth2List
  ...
  constructor: (@$container, @rolename) ->
    # gather elements
    @$display_table          = @$container.find('.auth2-list-table')
    @$request_response_error = @$container.find('.request-response-error')
    @$add_section            = @$container.find('.auth2-list-add')
    @$allow_field             = @$add_section.find("input[name='email']")
    @$allow_button            = @$add_section.find("input[name='allow']")

    # attach click handler
    @$allow_button.click =>
      @access_change @$allow_field.val(), 'allow', => @reload_auth2_list()
      @$allow_field.val ''
    ...
    load_auth2_list = (data) =>
      ...
      grid.onClick.subscribe (e, args) =>
        ...
        if args.cell is WHICH_CELL_IS_REVOKE
          @access_change item.email, 'revoke', => @reload_auth2_list()
...
class Students
  ...
  constructor: (@$section) ->
    ...
    # gather elements
    @$list_selector = @$section.find 'select#student-lists-selector'
    @$auth2_list_containers = @$section.find '.auth2-list-container'
    @$auth2_list_errors = @$section.find '.student-lists-management .request-response-error'

    # initialize & store Auth2List subsections
    # one for each .auth2-list-container in the section.
    @auth2_lists = _.map (@$auth2_list_containers), (auth2_list_container) =>
      rolename = $(auth2_list_container).data 'rolename'
      new Auth2ListWidget $(auth2_list_container), rolename, @$auth2_list_errors

    # populate selector
    @$list_selector.empty()
    for auth2_list in @auth2_lists
      @$list_selector.append $ '<option/>',
        text: auth2_list.$container.data 'display-name'
        data:
          auth2_list: auth2_list
    if @auth2_lists.length is 0
      @$list_selector.hide()
    ...
    @$list_selector.change =>
      ...
      for auth2_list in @auth2_lists
        auth2_list.$container.removeClass 'active'
      auth2_list = $opt.data('auth2_list')
      auth2_list.$container.addClass 'active'
      auth2_list.re_view()
