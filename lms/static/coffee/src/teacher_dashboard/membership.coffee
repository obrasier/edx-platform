    ...
    @get_member_list (error, member_list) =>
      ...
      _.each member_list, (member) =>
        ...
        $revoke_btn = $ _.template('<div class="revoke"><i class="icon fa fa-times-circle" aria-hidden="true"></i> <%= label %></div>', {label: label_trans}),
          class: 'revoke'
        $revoke_btn.click =>
            if confirm "Are you sure you want to remove this e-mail/account from your class? This will not delete the account, only remove this account from your administrative view."
